...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 *    => private 접근제한자로 선언된 멤버변수 instanace에 접근하여
...
//		SingletonClass sc2 = new SingletonClass();
		// => 기본 생성자가 public으로 되어 있어서 new 키워드를 통해서
		//    인스턴스를 여러개 생성이 가능!
		// => 기본 생성자 앞에 public이 아닌 private 접근제한자를 사용하여
		//    외부에서 new 키워드를 통한 인스턴스 생성을 하지 못하도록 설정
		//    즉, 외부에서 인스턴스 생성이 차단됨.
		
		// 클래스 내에서 인스턴스를 생성하여 멤버변수 instance에 저장했기 때문에
		// 해당 변수를 통해 인스턴스를 가져올 수 있다.
		// => 이 때, 해당 변수는 static 변수 이므로 클래스명만으로 접근 가능함
//		SingletonClass sc = SingletonClass.instance;
		
		// 주의! SingletonClass로 부터 리턴받은 인스턴스를 통해
		// 클래스 내의 Singleton 타입 변수 instance에 접근이 가능하므로
		// 해당 변수 값을 변경할 우려가 있음.
//		SingletonClass.instance = null;
//		sc.instance = null;
		// => 외부에서 멤버변수에 함부로 접근하지 못하도록
		//    private 접근제한자를 붙여서 외부 접근을 차단해야 함!
