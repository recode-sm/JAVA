...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *      		staic 메서드가 로딩되는 시점에서는 존재하지 않는다!
...
//		StaticMethod.normalMethod(); // 컴파일 에러
...
//		StaticMethod.setStaticVar(0);
		...
		sm.staticMethod(); // 정적 메서드 호출(단, 올바른 접근 방법은 아님)
...
//		System.out.println("정적 메서드에서 인스턴스 변수 접근 : " + normalVar);
		// => 컴파일 에러 발생
		...
		// static 메서드에서 일반 메서드 호출 불가능! (변수와 원인이 동일함)
//		normalMethod();		
	}
	
	// 인스턴스 변수 normalVar에 대한 Setter정의
...
//		this.staticVar = staticVar;
		// static 메서드 내에서 레퍼런스 this 사용 할 수 없음!
		// (Cannot use this in a static context)
		// => 레퍼런스 this 는 인스턴스 생성 시점에 생성됨.
