...
public class Ex {
	...
	public static void main(String[] args) {
		/*
		 * 객체 지향 프로그래밍(OOP: Object-Oriented Programmiong)
		 * => OOP is A.P.I.E
		 *    A(Abstraction): 추상화, P(Polymorphim) : 다형성
		 *    I(Inheritance) : 상속성, E(Encapsulation): 은닉성(캡슐화)
		 * 상속(Inheriatacne)
		 ...
		 * - 생성자는 상속 대상에서 제외됨
		 *   => 생성자의 이름은 자신의 클래스 이름어야함
		 *      상속받은 생성자는 부모의 클래스 이름으로 규칙에 위반되기 때문
		 * - 자바는 단일 상속만 지원
		...
		// 슈퍼클래스인 Parent 클래스의 멤버도 아무런 선어없이 사용 가능
		c.num = 10;
		c.parentPrn();
...
class Parent { // class Parent extends object와 동일함
	int num; // 슈퍼클래스의 멤버 변수
...
class Child extends Parent {
	...
	public void showNum() {
		...
		System.out.println("부모로부터 상속 받은 멤버변수 num = " + num);
	}
}
