...
public class Ex2 {
	...
	public static void main(String[] args) {
		...
		// 기본데이터타입 변수는 그대로 출력하면 저장된 정수데이터 출력됨
...
//		Integer n2 = new Integer(20); // 정수 20을 갖는 인스턴스를 생성하고
		...
		// Wrapper 클래스에는 toString() 메서드가 오버라이딩이 되어 있으므로
		// 객체가 저장한 값을 문자열로 리턴하여 데이터 출력이 가능함
		...
		// 기본 데이터타입 <-> 참조 데이터타입 변환 시 메서드 등을 활용해야했음
...
//		num2 = n2.intValue(); // Integer 객체 -> int 타입으로 변환하는 메서드
		...
		// => 기본 데이터타입과 Wrapper 클래스타입 간의 연산 수행할 경우
		//    Wrapper 클래스타입 객체를 오토 언박싱 후 연산을 수행함.
		
		System.out.println("----------------------------------------");
		
		// Object 타입 변수에 기본 데이터타입 저장 가능
		// => 단, 기본 데이터타입 -> Wrapper 클래스타입으로 오토박싱 후
