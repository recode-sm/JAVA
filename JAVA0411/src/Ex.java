...
//		p.name = "홍길동"; // 컴파일 에러 발생! The field Person.name is not visible
//		p.age = 20; // not visible
		// => 멤버변수에 저장할 값을 대신 전달할 Setter 메서드 호출
		p.setName("홍길동"); // 이름을 전달받아 초기화하는 setName)() 메서드 호출
		p.setAge(20); // 나이를 전달받아 초기화하는 setAge() 메서드 호출
		
		// 멤버변수 값을 출력하기 위해 참조변수를 통해 멤버변수 접근
		// => 멤버변수 접근제한자가 private 이므로 외부클래서 접근 불가!
