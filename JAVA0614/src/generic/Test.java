...
public class Test {
	...
	public static void main(String[] args) {
		// Person 클래스 인스턴스 2개를 생성 및 참조변수 출력
		// p1("홍길동",20) p2("이순신",44)
...
//		}
		 ...
		 *    Object 타입을 실제 데이터타입으로 다운캐스팅을 해야 한다!
		 */
		...
		// 문자열 "아이티윌" 추가
		list.add("아이티윌"); // Object 타입이므로 Person이 아닌 타입도 추가 가능
		// => Person 객체 형태로 꺼내서 사용하는 시점에서 문제가 발생 할 수 있다.
		
//		// 일반 for문을 통한 객체 출력
//		for(int i = 0; i < list.size(); i++) {
////			Person p = list.get(i);
//			// => 제네릭 타입을 사용하지 않아 list 는 Object로 되어 있다.
//			// => Object 타입을 Person 타입으로 대입하려고 하면 컴파일 에러가 나며
//			//    다운캐스팅이 필요함을 알 수 있다.
////			Person p = (Person)list.get(i);
////			System.out.println(p);
//			// => 실행을 시키면 86번 라인에서 에러가 발생한다.
//			// => java.lang.ClassCastException: java.lang.String cannot be cast to generic.Person
//			// => "아이티윌" 의 String 형태는 Person 객체로 변경할 수 없음을 알 수 있다.
//			// => 위의 코드 중 "list.add("아이티윌") 을 주석처리하고 다시 실행하면 오류가 없어짐
//			
//			// Person 타입으로 가져오기 전 타입 판별 후 형변환 수행
//			if(list.get(i) instanceof Person) {
////				Person p = list.get(i); // Object -> Person 다운캐스팅 필요
//				Person p = (Person)list.get(i);
////				System.out.println(p);
//				System.out.println(p.name + ", " + p.age);
//			}
...
//		}
		 ...
		 * - 또한, 제네릭타입 Person 타입으로 타입이 고정되므로
		 *   Object 타입으로 업캐시팅이 일어나지 않음
		 *   => 데이터를 꺼내는 시점에서도 Person 타입 그대로 사용 가능
		 *      (별도의 타입 체크 또는 다운캐스팅 작업 불필요)
		 */
		
		// <Person> 제네릭을 사용하는 list2 AarryList 생성
		...
		// 데이터 타입 판별 과정에서 오류(컴파일에러) 가 발생하게 된다!
//		list2.add("아이티윌"); // 컴파일 에러 발생
		// => The method add(Person) in the type List<Person> is not applicable for the arguments (String)
