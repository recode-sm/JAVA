...
public class Ex6 {
	...
	public static void main(String[] args) {
		 ...
		 *   => A 라는 쓰레드가 사용하는 데이터를
		 *      B 라는 쓰레드에서 동시에 접근하여 변경하는 경우
		 *      A 가 의도한 데이터가 아니게 될 수 있다!
		 * - 공유 데이터(객체)에 대한 일관성 문제를 해결하기 위해
		 *   데이터베이스의 Lock 개념과 유사한 동기화(Synchronized) 기능을 사용
		...
		// 은행 계좌 1개를 생성 - 잔고 10000원
		Account account = new Account(10000);
		
		// 출금 작업을 수행하기 위한 쓰레드 구현 객체 2개를 생성
...
//	public int withdraw(int amount) {
		...
		// 불가능할 경우 잔액 부족 메시지 출력 및 출금금액을 0으로 변경
