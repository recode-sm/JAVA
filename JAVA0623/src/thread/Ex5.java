...
public class Ex5 {
	...
	public static void main(String[] args) {
		 ...
		 *   (단, 절대적인 수치는 아니고 확률적으로 실행 비중을 높여줌)
		 * - Thread 객체의 getPriority() 메서드 : 우선순위 조회
		 * - Thread 객체의 setPriority() 메서드 : 우선순위 결정
		 *   => 우선순위는 1 ~ 10까지 범위의 정수를 사용
		 ...
		 *   해당 쓰레드는 잠시 일시 정지 상태가 되어 Waiting pool로 이동함
		 * - sleep() 메서드 파라미터로 밀리초 단위 또는 나노초 단위의
		 *   시간을 설정하면 해당 시간이 만료되면 다시 실행기 상태로 변경됨
		 ...
		 *   쓰레디 상태 저장 및 자원 교환에 필요한 시간이 추가되므로
		 *   지정된 시간보다 약간의 델레이가 포함될 수 있다!      
		 *   
		 * < 기분 문법 >
		 * try {
		 * 		Thread.sleep(밀리초);
		 * } catch (InterruptedExcetption e) {
		 * 		// interrupt() 메서드가 호출될 때(쓰레디가 깨어날 때)
		 *		// 실행할 작업을 기술..
		 * }
		 *      
		 */
