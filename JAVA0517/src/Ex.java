...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *               클래스 하나 당 하나만 생성해도, 모든 인스턴스가 공유함
		 *               => 클래스명만으로도 접근 가능(클래스명.변수명)
		...
		// 값을 비교할 때도 상수명만으로 비교하면되므로 오류 발생 확률 없음
...
class CarInfoWithConstant {
	 ...
	 * 문자열 오타나, 정수형 파라미터 사용 시 등의
	 * 다양한 어려움에서 오는 문제점을 해결 할 수 있다!
	 * => 주로 상수는 public static final 키워드 사용
	 *    public에 의해 어디서든 접근 가능하며,
	 *    static에 의해 클래스명만으로 인스턴스 생성없이 접근 가능하고,
	 *    final에 의해 데이터가 변경되지 않음
	 *    => 상수의 이름을 명확하게 지정함으로써
	 *       복잡한 데이터도 이름만으로 단순히 사용이 가능하게 됨!
	 */
	
	// 1. 제조자 정보를 문자열로 저장하는 상수 선언
	...
	public static final int COLOR_BARCELLONA_RED = 0xFF0000;
