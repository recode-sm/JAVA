...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		LocalTime time = LocalTime.now(); // 시스템의 시각 정보를 리턴받음
		System.out.println(time);
		
		LocalDateTime dateTime = LocalDateTime.now(); // 날짜, 시각 정보를 리턴받음
		...
		LocalTime time2 = LocalTime.of(17, 50, 01); // 시, 분, 초 정보 설정
...
//		String monthName = enumMonth; // 컴파일 오류 발생
		// => Type mismatch: cannot convert from Month to String
		// => 문자열 변수에 저장 시 String 변환 필수! => toString()
...
//		System.out.println(enumMonth.getDisplayName(TextStyle.SHORT, Locale.ENGLISH));
		...
		// => 년도를 비교하고 같은 년도면 월을 비교, 같은 월이면 일수를 비교
