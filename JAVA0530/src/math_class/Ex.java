...
public class Ex {
	...
	public static void main(String[] args) {
		...
		System.out.println(Math.ceil(dNum)); // 4.0 , 소수점 첫째자리 올림
		System.out.println(Math.floor(dNum)); // 3.0 , 소수점 첫째자리 버림
		System.out.println(Math.round(dNum)); // 3 , 소수점 첫째자리 반올림(정수 리턴)
		
		/*
		 * round() 메서드는 항상 소수점 첫째자리에서 반올림이 수행되는데
		 * x번째 자리에서 반올림을 수행하기 위해서는 반올림 할 숫자를
