...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *      (첫글자 인덱스 0번, 마지막 글자 인덱스 문자열 길이-1)
		 *      
		 * < String 클래스의 메서드 >
		...
		// equlas() : 문자열 비교(대소문자 구별하여 비교)
		System.out.println("s1.equlas(JAVA PROGRAMMING) : " + s1.equals("JAVA PROGRAMMING"));
		
		// equlasIgnoreCase() : 문자열 비교(대소문자 구별하지 않고 비교)
		System.out.println("s1.equlasIgnoreCase(JAVA PROGRAMMING) : " + 
		...
		// 1) substring(int beginIndex) : 시작 인덱스로부터 끝까지 문자열 추출
		System.out.println("s1.substring(5) : " + s1.substring(5));
		// => 5번 인덱스(P)부터 마지막 문자열까지 모두 추출 : "Progamming"
		
		// 2) substring(int beginIndex, int endIndex) : 시작인덱스 ~ 끝인덱스-1 까지 추출
...
//		String s3 = "자바/JSP/안드로이드";
		...
		// 만약, "." 기호를 구분자로 지정하는 경우
		// 정규표현식에서 특수기호로 사요되어 모든 문자를 구분자로 지정하게 됨
