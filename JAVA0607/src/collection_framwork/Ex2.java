...
public class Ex2 {
	...
	public static void main(String[] args) {
		...
		/*
		 * <Iterator(반복자) 사용 기분 문법>
		 * 1. 컬렉션 객체(Set 또는 List)의 iterator() 메서드를 호출하여
		 *    Iterator 타입 객체를 리턴 받음
		 * 2. while 문을 통해 Iterator 객체의 hasNext() 메서드가 true일 동안 반복
		 * 3. 반복문 내에서 Iterator 객체의 next() 메서드를 통해 데이터 가져오기
		 */
		
		Iterator ite = set.iterator(); // Iterator(인터페이스) 객체 얻어오기
...
//			System.out.println(ite2.next()); // 다음 요소를 꺼내서 출력
			int num = ite2.next();
			System.out.println("set 객체에 " + num + "이(가) 존재하는가? " + set.contains(num));
		}
